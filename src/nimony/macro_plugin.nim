...
proc rewriteSymsToIdentsImpl(newBuf: var TokenBuf; n: var Cursor) =
  ...
  case n.kind
  of Symbol, SymbolDef:
    var name = pool.symString(n.symId)
...
proc buildPluginNif*(macroDecl: Cursor; macroSym: SymId;
  ...
  let macroName = sourceIdent(pool.symString(macroSym))
...
proc compileMacroPlugin*(nifcachePath: string; macroDecl: Cursor; macroSym: SymId;
  ...
  try:
    ...
    return ""
  if exitCode != 0:
    echo "Error compiling macro plugin for '", sourceIdent(pool.symString(macroSym)), "':"
    echo output
    return ""
