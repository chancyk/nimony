...
| compiler, cold | 5.45 s | 5.41 s | 116 / 147 MB |
...
| 6 | **Incremental assembly** (B3/B3c/B3d): nifasm caches emitted fragments per symbol, validated per declaration (asm-NIF has no line info, so a declaration's bytes are its identity); foreign procs resolved from their signature; arkham names labels and temps per proc so unchanged procs assemble byte-identically | nativenif `blobcache.nim`, `core/declhead.nim`, `core/asmdecls.nim`, arkham `CodeGen.enterProc` | the DCE worklist is untouched (same pops, same order); sections byte-identical in nine cache states; 127-module warm link 0.84 s → 0.086 s, and a one-proc edit re-records 18 fragments instead of 627 | nativenif `blobcache_selftest` |
