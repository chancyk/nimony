...
| B3d | running: per-symbol blob validity in nifasm (627 -> ~3 stale fragments on a live `sem.nim` edit) and per-proc asm splicing in arkham, keyed on the `.decls.nif` lowering-output digests | |
| B4, B5 | planned | |
