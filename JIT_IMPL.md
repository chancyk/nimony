...
| B3e | running: arkham per-proc splice (re-lower only changed procs, splice the previous `.asm.nif`; needs incremental `.c.nif` parse and `.asm.nif` serialize to reach ~0.1 s) | |
